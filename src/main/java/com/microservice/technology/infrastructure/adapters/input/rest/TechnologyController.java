...
import com.microservice.technology.infrastructure.adapters.input.rest.mapper.Tech_CapacityRestMapper;
...
public class TechnologyController implements TechnologyAPI {
    ...
    private final Tech_CapacityRestMapper tech_CapacityRestMapper;
    ...
    public Mono<Boolean> existsById(int id) {
        return servicePort.existsById(id);
    }

    @Override
    public Mono<CreateTechResponse> findByName(String name) {
        return servicePort.findByName(name).map(mapper::fromTechonologyToCreateTechResponse);
    }

    @Override
    public ResponseEntity<Mono<Void>> createTechCapacity(CreateTechCapacityRequest request) {
        return new ResponseEntity<>(servicePort.createTechCapacity(request.capacity_id(), request.technologies()), HttpStatus.OK);
    }
}
